 ...
 */
export async function getWorkersSubdomain(apiToken: string, accountId: string): Promise<string> {
 ...
 */
export async function createR2Bucket(apiToken: string, accountId: string, bucketName: string): Promise<string> {
  ...
  const data = (await res.json()) as any;
  if (!res.ok && !data.errors?.some((e: any) => e.code === 10006 || e.message?.includes('already exists'))) {
 ...
 */
export async function enableWorkerSubdomain(apiToken: string, accountId: string, workerName: string): Promise<void> {
